...
This is a stage-3 spike helper: it works directly against docling's raw
native export (see docs/examples/docling/*/native.json on
origin/docling-example-outputs) rather than against a stage-2 normalized.json,
because stage 2 (pipeline/processing, per prd.md) does not exist yet.

Known docling quirk (see CONTEXT.md / ADR for stage 3): cell `text` for
Hebrew table content is sometimes word-order-reversed (e.g. "המים מפעל"
instead of "מפעל המים") even though header cells usually come out correct.
This module does not attempt to fix that -- the category-classification
LLM call is expected to be robust to it, and any resulting low-confidence
matches are a legitimate signal, not something to silently paper over.
"""
...
def _detect_unit(all_cells: list[dict], doc_texts: list[str]) -> tuple[str, bool, str | None]:
    # Deliberately scans every cell, not just column_header-flagged ones:
    # docling's column_header flag is unreliable on OCR'd tables (see
    # mate_yehuda_2024, where the unit row's flag comes out False and the
    # row is otherwise indistinguishable from a data row).
    for c in all_cells:
...
def extract_table(doc: dict, table_index: int) -> TableExtract:
    ...
    # docling's column_header flag is unreliable on OCR'd tables (see
    # mate_yehuda_2024) -- row 0 is always treated as the header row too,
