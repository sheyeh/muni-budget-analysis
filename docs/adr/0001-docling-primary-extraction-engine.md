...
**Decision**: no single product wins on both structure and text accuracy, so the fix is a hybrid, not a product swap — keep docling's TableFormer for table geometry (the one component that reliably paired rows/columns on both scanned files), and replace only the OCR text-recognition step feeding it (Tesseract → Cloud Vision), e.g. via docling's pluggable `ocr_options` or a per-cell-crop post-process using docling's own layout bounding boxes. Not yet implemented — this is the concrete next step for `router.py`'s `docling_pdf_ocr` path (owned by `worktree-level2-pipeline`), not this spike branch. Cost is not a constraint (GCP credits) and a large share of the ~200-file corpus is scanned, so this is worth doing before Task 7's full batch run.
